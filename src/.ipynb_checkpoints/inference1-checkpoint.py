...
sparse_features = ['userid', 'feedid', 'device', 'authorid', 'bgm_song_id', 'bgm_singer_id'] + [
    'cluster_0', 'cluster_1',
       ...
       'cluster_3_authorid', 'cluster_4_authorid', 'cluster_5_authorid',
               'cluster_0_user', 'cluster_1_user', 'cluster_2_user',
       'cluster_3_user', 'cluster_4_user', 'cluster_5_user'    
]
...
vocab = {'userid': 250249, 'feedid': 112872, 'device': 3, 'authorid': 18789, 'bgm_song_id': 25160, 'bgm_singer_id': 17501, 'manual_keyword_list': 27271, 'machine_keyword_list': 27264, 'manual_tag_list': 353, 'machine_tag_list': 346,
        ...
        'cluster_0_user': 10, 'cluster_1_user': 100, 'cluster_2_user': 500, 'cluster_3_user': 1000, 'cluster_4_user': 50, 'cluster_5_user': 5000,
        }
max_len = {'manual_keyword_list': 5, 'machine_keyword_list': 5, 'manual_tag_list': 5, 'machine_tag_list': 3}   

feed_df = pd.read_csv(BASE_DIR[:-3] + 'data/feature/userid_cluster.csv')
test = pd.merge(test, feed_df, on='userid', how='left')
...
for k in [i for i in range(9)] + [i for i in range(23, 29)] + [i for i in range(33, 38)]:  
...
# test[['userid', 'feedid'] + targets].to_csv(BASE_DIR[:-3] + f'data/submission/result_2.csv', index=None, float_format='%.6f')

res1 = pd.read_csv(BASE_DIR[:-3] + f'data/submission/result_1.csv')
for t in targets:
    res1[t] = res1[t] * 0.4 + test[t] * 0.6
res1.to_csv(BASE_DIR[:-3] + f'data/submission/result.csv', index=None, float_format='%.6f')
